  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 animate-in fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl p-4 border-2 border-emerald-200 shadow-sm space-y-3">
          
          {/* Active Booking Origin & Destination Status Banner */}
          {(activeRide || incomingDriverRequest) ? (
            <div className="p-3.5 bg-gradient-to-r from-emerald-950 via-slate-900 to-emerald-950 text-white rounded-2xl border border-emerald-500/50 shadow-md space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-black tracking-wider uppercase text-emerald-400">
                    {incomingDriverRequest ? '⚡ NEW INCOMING BOOKING ALERT' : '🚗 ACTIVE ASSIGNED RIDE'}
                  </span>
                </div>
                <div className="px-2 py-0.5 bg-emerald-500/20 border border-emerald-400/40 rounded-full text-[10px] font-black text-emerald-300 font-mono-num">
                  FARE: ₹{(activeRide || incomingDriverRequest)?.finalFare}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs pt-1 border-t border-white/10">
                <div className="bg-white/10 rounded-xl p-2 border border-white/10">
                  <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wide flex items-center gap-1">
                    ...
                    KAHAN SE ORDER BOOKING HUA (PICKUP):
                  </span>
                  <p className="font-extrabold text-white leading-tight mt-0.5 truncate" title={(activeRide || incomingDriverRequest)?.pickup.address}>
                    {(activeRide || incomingDriverRequest)?.pickup.address}
                  </p>
                </div>

                <div className="bg-white/10 rounded-xl p-2 border border-white/10">
                  <span className="text-[10px] font-bold text-orange-400 uppercase tracking-wide flex items-center gap-1">
                    <Flag className="w-3 h-3 text-orange-400" />
                    KAHAN JANA HAI (DROP DESTINATION):
                  </span>
                  <p className="font-extrabold text-white leading-tight mt-0.5 truncate" title={(activeRide || incomingDriverRequest)?.destination.address}>
                    {(activeRide || incomingDriverRequest)?.destination.address}
                  </p>
                </div>
              </div>
            </div>
          ) : (
            <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-200 text-xs font-bold text-emerald-950 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-600 animate-pulse"></span>
                <span>Driver GPS Ready • Watching Golaghat, Bokakhat & Kaziranga corridors</span>
              </div>
          ...
          <div className="flex items-center justify-between p-2 bg-slate-50 rounded-2xl border border-slate-200">
            <div className="flex items-center gap-1.5 px-2 text-xs font-display font-black text-slate-800">
              ...
              <span>DRIVER LIVE GPS NAVIGATION:</span>
            </div>
          ...
          {mapEngine === 'google' ? (
            <GoogleMapsEmbed
              pickup={activeRide?.pickup || incomingDriverRequest?.pickup}
              destination={activeRide?.destination || incomingDriverRequest?.destination}
            ...
            <InteractiveMap
              pickup={activeRide?.pickup || incomingDriverRequest?.pickup}
              destination={activeRide?.destination || incomingDriverRequest?.destination}
              driverLocation={activeRide?.driverLocation || driverProfile?.currentLocation}
              vehicleType={driverProfile?.vehicleType || 'cab'}
              status={activeRide?.status || (incomingDriverRequest ? 'driver_assigned' : undefined)}
              heightClass="h-[460px] sm:h-[520px]"
            />
          )}
        </div>
      ...
      {incomingDriverRequest && !isFeeLocked && (
        <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4 animate-in zoom-in-95">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border-2 border-emerald-500 space-y-4 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between pt-1">
              <div className="flex items-center gap-2">
                <span className="w-3.5 h-3.5 rounded-full bg-orange-600 animate-ping" />
              ...
              <div className="text-right">
                ...
                <span className="text-[10px] text-slate-500 block font-display font-black uppercase">Upfront Fare</span>
              </div>
            </div>

            {/* HIGH-VISIBILITY BOX: KAHAN SE ORDER AAYA HAI (PICKUP ORIGIN) */}
            <div className="p-4 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl border-2 border-emerald-400 space-y-3 text-xs shadow-xs">
              <div className="flex justify-between items-center pb-2 border-b border-emerald-200">
                <div>
                  <span className="text-[10px] font-black text-emerald-800 uppercase tracking-wider block">PASSENGER NAME</span>
                  <span className="font-display font-black text-slate-950 text-base">{incomingDriverRequest.customerName}</span>
                </div>
                <div className="text-right">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider block">TRIP DISTANCE</span>
                  <span className="font-display font-black text-orange-600 font-mono-num text-sm">{incomingDriverRequest.distanceKm} km</span>
                </div>
              </div>

              {/* Exact Pickup Origin Location Details */}
              <div className="p-3 bg-white rounded-xl border border-emerald-300 shadow-2xs space-y-1">
                <div className="flex items-center gap-1.5 text-emerald-800 font-black text-[11px] uppercase tracking-wide">
                  <MapPin className="w-4 h-4 text-emerald-600" />
                  <span>KAHAN SE ORDER BOOKING HUA (PICKUP POINT):</span>
                </div>
                <p className="text-sm font-black text-slate-950 pl-5 leading-snug">
                  {incomingDriverRequest.pickup.address}
                </p>
                {incomingDriverRequest.pickup.landmark && (
                  <p className="text-xs font-bold text-emerald-700 pl-5">
                    Landmark: {incomingDriverRequest.pickup.landmark}
                  </p>
                )}
              </div>

              {/* Destination Drop Location Details */}
              <div className="p-3 bg-white rounded-xl border border-orange-200 shadow-2xs space-y-1">
                <div className="flex items-center gap-1.5 text-orange-800 font-black text-[11px] uppercase tracking-wide">
                  <Flag className="w-4 h-4 text-orange-600" />
                  <span>KAHAN JANA HAI (DROP DESTINATION):</span>
                </div>
                <p className="text-sm font-black text-slate-950 pl-5 leading-snug">
                  {incomingDriverRequest.destination.address}
                </p>
              </div>
            </div>

            {/* Compulsory PIN notice */}
            <div className="px-3 py-2 bg-amber-50 rounded-xl border border-amber-200 text-[11px] font-bold text-amber-900 flex items-center gap-2">
              <KeyRound className="w-4 h-4 text-amber-700 shrink-0" />
              <span>Customer will share a 4-Digit PIN when you arrive at pickup to start the ride.</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
              ...
              <button
                ...
                className="py-3.5 bg-gradient-to-r from-emerald-600 to-orange-500 hover:from-emerald-700 hover:to-orange-600 text-white text-xs font-display font-black uppercase tracking-wider rounded-2xl shadow-xl shadow-emerald-500/25 transition-transform active:scale-95 cursor-pointer flex items-center justify-center gap-1.5"
              >
                <CheckCircle2 className="w-4 h-4" />
                <span>ACCEPT BOOKING ({countdownSeconds}S)</span>
              </button>
            </div>
          </div>
        </div>
